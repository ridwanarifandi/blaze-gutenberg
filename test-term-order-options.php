...
function test_term_order_options_direct()
{
    ...
    // Test orderby handling without dependency check
    $test_orderby = 'term_order';
    $fallback_result = blaze_get_fallback_orderby($test_orderby);
    $should_use_result = blaze_should_use_term_order($test_orderby);

    error_log("Test orderby 'term_order':");
    error_log("  - Fallback result: " . $fallback_result);
    error_log("  - Should use result: " . ($should_use_result ? 'YES' : 'NO'));
    error_log("  - Expected: term_order should pass through without change");
